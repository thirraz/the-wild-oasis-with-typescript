...
export function BookingMenu({ bookingId, status }: Props) {
	...
	return (
		<Modal>
			<div className="absolute top-3 right-[80%] bg-[#000]  text-grey-600 rounded-sm border border-grey-300 transition-all">
				<button
					onClick={() => navigate(`/bookings/${bookingId}`)}
					className="flex justify-center bg-grey-0 items-center px-14 py-3 gap-4 w-full hover:bg-grey-100"
				>
				...
				<Modal.Open openWindowName="delete">
					<Button
						color="danger"
						className="w-full text-red-100 rounded-[0px]"
					>
						<HiTrash /> Delete
					</Button>
				</Modal.Open>
